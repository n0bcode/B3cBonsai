...
namespace B3cBonsaiWeb.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                // Điều này không tính các lần đăng nhập thất bại vào việc khóa tài khoản
                // Để kích hoạt việc khóa tài khoản khi đăng nhập sai, đặt lockoutOnFailure: true
                var result = await _signInManager.PasswordSignInAsync(Input.Email, Input.Password, Input.RememberMe, lockoutOnFailure: false);
                if (result.Succeeded)
                {
                    // Lấy người dùng hiện tại từ Email
                    ...
                    if (user != null)
                    {
                        ...
                        {
                            return LocalRedirect(returnUrl); // Trang mặc định cho User
                        }
                    }
                }
            ...
            // Nếu chúng ta đã đến đây, điều gì đó đã thất bại, hiển thị lại form
            return Page();
        }
    }
}
